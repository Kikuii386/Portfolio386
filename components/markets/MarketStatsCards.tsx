...
export default function MarketStatsCards() {
  ...
  const Card = ({ title, icon: Icon, data, type }: any) => (
    <div className="bg-white border border-[#F5F2EB] rounded-2xl p-5 shadow-sm">
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-[#4A4A48] font-bold flex items-center gap-2">
          <Icon
            size={18}
            className={
              type === 'hot'
        ...
        <button className="text-xs text-[#A4AC86] hover:text-[#606c38] flex items-center">
        ...
        {data.map((coin: any, i: number) => (
          ...
          >
            <div className="flex items-center gap-3">
              <span className="text-xs text-[#C7BFB1] font-mono w-4">
                {i + 1}
              </span>
              ...
              <div className="flex flex-col">
                <span className="text-sm font-bold text-[#4A4A48] group-hover:text-[#A4AC86] transition-colors">
                  {coin.name}
                </span>
                <span className="text-[10px] text-[#C7BFB1]">
                  {coin.symbol}
                </span>
              </div>
            </div>
            <div className="text-right">
              <div className="text-xs font-mono text-[#4A4A48]">
                ${coin.price.toLocaleString()}
              </div>
              <div
                className={`text-[10px] font-bold ${coin.change >= 0 ? 'text-[#606c38]' : 'text-[#bc4749]'}`}
              >
                {coin.change >= 0 ? '+' : ''}
                {coin.change}%
              </div>
